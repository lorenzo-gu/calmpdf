...
import { docxToPdf } from "@/lib/conversion";
...
export function DocxToPdfTool() {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const filename = file.name.replace(DOCX_EXT, "") + ".pdf";
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-sage-700">
        Tip: this first version works best for simple DOCX files. Complex formatting may look slightly different in the PDF.
      </p>
      ...
      {result && (
        <div className="card bg-sage-50 border-sage-100">
          ...
          <p className="mt-2 text-xs text-sage-700">
            Original: {formatBytes(result.originalSize)} · PDF: {formatBytes(result.outputSize)}
          </p>
        </div>
      )}
